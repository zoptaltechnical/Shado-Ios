...
- (id)init
{
    ...
    if (self) {
        ...
        _hideAllButtons = NO;
        _hideDoneButton = NO;
        _avoidCustomImageTint = YES;
        _blurBackground = NO;
        _bounceAnimations = NO;
        _darkTheme = NO;
        _detachButtons = NO;
        _fullCircleCustomImage = YES;
...
- (void)drawRect:(CGRect)rect {
    ...
    if (alertViewWithVector) // Frames for when AlertView contains an image
        alertViewFrame = CGRectMake(self.frame.size.width/2 - ((result.width - defaultSpacing)/2),
                                    self.frame.size.height/2 - (400.0f/2),
    ...
    // HERE I HAVE CHANGE THE POSITION OF SUBTITLE
    UILabel *descriptionLabel = [[UILabel alloc] initWithFrame:CGRectMake(15.0f,
                                                                          descriptionLevel + (alertViewWithVector * 55),
...
//                                                                          60.0f)];
    ...
    if (_title != nil)
        descriptionLabel.font = [UIFont fontWithName:@"Cabin-Regular" size:16];
    else
        descriptionLabel.font = [UIFont fontWithName:@"Cabin-Regular" size:16];
    ...
    titleLabel.font = [UIFont fontWithName:@"RobotoSlab-Regular" size:22];
    ...
    separatorLineView.backgroundColor = [UIColor colorWithWhite:100.0f/255.0f alpha:1.0];
    ...
    if (_numberOfButtons == 0) { // View only contains DONE/DISMISS Button
        ...
        // Here i have made the changes for colour :
        if (_detachButtons)
    ...
    // I have made the changes at here : to hide the third button
        ...
        if (_detachButtons)
            ...
            // Here i have added the colour for Yes Button of alert.
            
        [secondButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];        
        secondButton.backgroundColor =   [UIColor colorWithRed:249.0/255.0 green:76.0/255.0 blue:86.0/255.0 alpha:1];
        if (_darkTheme)
        ...
        if (_detachButtons) {
            secondButton.frame = CGRectMake(secondButton.frame.origin.x + 25,
                                            secondButton.frame.origin.y - 25,
                                            secondButton.frame.size.width - 50,
                                            55);
        ...
        [doneButton setTitleColor:[UIColor colorWithRed:249.0/255.0 green:76.0/255.0 blue:86.0/255.0 alpha:1] forState:UIControlStateNormal];

        doneButton.titleLabel.font = [UIFont fontWithName:@"Cabin-Regular" size:16];
        if (_colorScheme != nil || _darkTheme)
            doneButton.tintColor = [UIColor whiteColor];
        if (self.doneButtonTitleColor != nil)
    ...
    CGFloat vectorSize = 45.0f * MIN(2, _customImageScale);
...
- (void) makeAlertTypeWarning {
    vectorImage = [self loadImageFromResourceBundle:@"ques@x"];
    alertViewWithVector = 1;
